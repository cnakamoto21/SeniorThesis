...
## read in phenotype data, order and filter based on alignment and antibiotic data availability
...
## read in single alignment from file, filter, generate binary encoded matrix
...
## call phenotype and alignment cleaners to generate binary matrices, phenotypes, species labels for cleaned subset of isolates
...
def grmer(splabs, mutlist, phens, reflist, genenames, litsites, anti, species = None, sp = 1, refsp = 1):
    ...
    with open(f"gemma_trial/{anti}_inter_{species if species != None else 'all'}.phenotypes", 'w') as myfile:
    ...
    ## BUILD THE GRM
    # drop the sites that are important
    impsites = [np.argwhere(sitenum == i)[0][0] for i in litsites if np.argwhere(sitenum == i).shape[0]>0]
    newmutlist[0] = np.delete(newmutlist[0], impsites, 1)
    if species: 
        ...
        for spcs in ["E.coli", "M.tb", "S.enterica", "S.aureus"]:
            ...
            for a in range(newmutlist[0].shape[0]): 
                if splabs[a] == spcs:
                    bcnt = 0
                    for b in range(newmutlist[0].shape[0]):
                        if splabs[b] == spcs:
                            grm[a,b] = minigrm[acnt, bcnt]
                            bcnt += 1
                    acnt += 1
    np.savetxt(f"gemma_trial/{anti}_dropsite_{species if species != None else 'all'}.grm.txt", grm, delimiter="\t")
               
    ## BUILD THE GRM - classic!!
    if species: 
        tempmuts = np.concatenate(newmutlist, 1)
        # p = np.mean(tempmuts, axis = 0)
        # tempmuts = tempmuts - np.tile(p, (tempmuts.shape[0],1))
                ...
                if splabs[a] == spcs:
                    ...
                    for b in range(mutlist[0].shape[0]):
                        if splabs[b] == spcs:
                            ...
                            bcnt += 1
                    acnt += 1
    np.savetxt(f"gemma_trial/{anti}_classic_{species if species != None else 'all'}.grm.txt", grm, delimiter="\t")
    ...
    indlist = []
    if not species:
        speclist =  ["E.coli", "M.tb", "S.enterica"]
        if anti == "rif":
            speclist = ["M.tb"]
        for spc in speclist:
        ...
        np.savetxt(f"gemma_trial/{anti}_inter_{species if species != None else 'all'}.covariates", inds, delimiter="\t")
    ...
    fileobj = open(f"gemma_trial/{anti}_inter_{species if species != None else 'all'}.loci", "w")
...
## generate table with information on each position for future analysis
def table_info(mutlist, sites, sitenums, anti, genes, species = None, inds = None): 
    counts = []
    for i in range(len(sites)):
        genenum = genes.index(sites[i].split(".")[0])
        idx = sitenums[i] - sum([mutlist[i].shape[1] for i in range(genenum)])
        if species: 
            sp = speciestosp[species]
            if sp < 3: 
                counts.append(sum(mutlist[genenum][inds[:,sp] == 1][:,idx]))
            else: 
                counts.append(sum(mutlist[genenum][(inds[:,0]==0)&(inds[:,1]==0)&(inds[:,2]==0)][:,idx]))
        else: 
            counts.append(sum(mutlist[genenum][:,idx]))
...
allgyrmuts, allgyrsitenums, allgyrsites = grmer(gyrinds, gyrmutlist, gyrphens, gyrreflist, gyrgenes,
                                                 [174, 178], "cip", species = None, sp = 0, refsp = 0)
table_info(gyrmutlist, allgyrsites, allgyrsitenums, "cip", gyrgenes, species = None, inds = None)
...
ref_sites = [305, 1018, 1013, 1020, 998, 994, 1008, 992]
allrifmuts, allrifsitenums, allrifsites = grmer(rifinds, rifmutlist, rifphens, rifreflist, rifgenes,
                                                 ref_sites, "rif", species = None, sp = 1, refsp = 1)
table_info(rifmutlist, allrifsites, allrifsitenums, "rif", rifgenes, species = None, inds = None)
